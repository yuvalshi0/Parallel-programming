...
object ParallelCountChange extends ParallelCountChangeInterface {
  ...
  def countChange(money: Int, coins: List[Int]): Int = coins match {
    case _ if (money == 0) => 1 // got to zero
    case _ if (money < 0)  => 0 // less then zero
    case Nil               => 0 // no more coins
    case x :: rest =>
      countChange(money, rest) + countChange(money - x, x :: rest)
  }
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int =
    coins match {
      ...
      case _ if (money < 0)  => 0 // less then zero
      case Nil               => 0 // no more coins
      case x :: rest if (threshold(money, coins)) => {
        val (a, b) = parallel(
          parCountChange(money, rest, threshold),
          parCountChange(money - x, x :: rest, threshold)
        )
        a + b
      }
      case x :: rest =>
  ...
  def moneyThreshold(startingMoney: Int): Threshold =
    (money, _) => startingMoney * 2 / 3 >= money

  /** Threshold heuristic based on the total number of initial coins. */
  def totalCoinsThreshold(totalCoins: Int): Threshold =
    (_, coins) => totalCoins * 2 / 3 >= coins.length
