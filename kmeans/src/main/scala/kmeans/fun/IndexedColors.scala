...
class IndexedColorFilter(initialImage: Img,
  ...
  private def imageToPoints(img: Img): ParSeq[Point] = {
    val width = 0 until img.width
    val height = 0 until img.height
    val points = width zip height
    ...
    val initialPoints: ParSeq[Point] =
      initStrategy match {
        ...
        case UniformChoice =>
          ...
          (0 until 256 by d).zip(0 until 256 by d).zip(0 until 256 by d).par.map {
            case ((x, y), z) => new Point(x.toDouble / 256, y.toDouble / 256, z.toDouble / 256)
          }
      }
